...
# has: fixtures from namespace metadata, assertions outside a test var, and
# test-ns-hook.
# ---------------------------------------------------------------------------
...
EOF
set +e
out="$(cd "$proj_ct1" && LGX_HOME="$home_ct1" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct2" && LGX_HOME="$home_ct2" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct3" && LGX_HOME="$home_ct3" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct4" && ORDER_LOG="$proj_ct4/order.log" LGX_HOME="$home_ct4" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct5" && LGX_HOME="$home_ct5" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct6" && LGX_HOME="$home_ct6" "$LGX" test 2>&1)"; rc=$?
set -e
...
EOF
set +e
out="$(cd "$proj_ct7" && LGX_HOME="$home_ct7" "$LGX" test 2>&1)"; rc=$?
set -e
...
# ---------------------------------------------------------------------------
echo "==> Scenario 159: one git dep reached with and without .git does not warn"
groot="$(mktemp -d)"
home_g="$(mktemp -d)"
