...
- [x] **Step 1: Replace the port default in `cmd-nrepl`**
...
- [x] **Step 4: Smoke-test the free-port path**
...
- [x] **Step 1: Reword the `parse-nrepl-args` docstring**
...
- [x] **Step 2: Rename the parse test**
...
- [x] **Step 2: ARCHITECTURE `lgx nrepl` section**
...
- [x] **Step 3: Close the deferred follow-ups in the issue**
...
Done. `cmd-nrepl` in `lgx.lg` now defaults the nREPL port to `(os/free-port)`
instead of `(+ 49152 (rand-int 16384))`; the comment, usage row, README,
ARCHITECTURE rationale, the `cli/parse-nrepl-args` docstring, and the renamed
parse test (`nrepl-no-args-means-auto-port`) all follow. The deferred lgx
follow-ups in `docs/issues/nrepl-port-zero.md` are marked done.

**Verification:** unit tests 436 tests / 591 assertions green (both lg builds);
a piped-EOF `lgx nrepl` smoke run produced a real free port in `.nrepl-port`
matching the banner, with no `failed to run` line; codex second-opinion review
found no issues.

**Issue hit (environment, not the change):** `make test` initially failed two
e2e scenarios unrelated to this change — both reproduce on a clean tree. The
installed PATH lg (`1.11.0`, commit `f9048d8`, mise shim) does not resolve
`*command-line-args*` through lgx's `run` path (fails Scenario 21), while the
checked-in `let-go/.tmp/lg` is an unstamped `dev` build, so lgx's version check
skips and the mismatch-warning test can't fire (fails Scenario 4b). Resolved by
building a version-stamped binary from let-go HEAD
(`go build -ldflags="-X main.version=1.11.0 -X main.commit=<sha>"`) and running
`make test LG=<that> LGX_LG=<that>` → **436 unit + 278 e2e assertions, all
passed.** No source change was needed for either e2e failure; they are a
let-go-build/tooling concern, and `lgx nrepl` has no e2e coverage regardless.
