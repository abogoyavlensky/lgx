...
- **`depends_on` placement:** after the `on_linux` block, before
  `def install` — the standard Homebrew component order, and where
  `brew style` expects it.
...
- [ ] **Step 3: Style-check the generated formula**
  Run: `brew style "$scratch/lgx.rb"`
  Expected: no offenses. If `brew style` flags component order, adjust
  the `depends_on` placement in the *generator* (not the generated
  file) and regenerate.
...
- [ ] **Step 2: README Homebrew install section**
...
- [ ] **Step 4: scripts/install.sh help text**
...
- [ ] **Step 5: Verify no stale command remains in the lgx repo**
...
- [ ] **Step 1: Edit the callout paragraph**
...
- [ ] **Step 1: Review the full diff**
  ...
  `+depends_on` and the removed `caveats` block.
