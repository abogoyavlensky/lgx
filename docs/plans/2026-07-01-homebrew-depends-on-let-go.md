...
> **Status: SUPERSEDED** (2026-07-01). The `depends_on` approach was
> implemented and verified, then reverted after a real `brew install`
> test — Homebrew 6's tap-trust gate errors on first install. Final
> approach: simplified `caveats` + direct user install. See summary at
> the end.
...
The `depends_on` approach was implemented, verified with `brew style`, and
reviewed — then **reverted** after a real `brew install` test. The plan's
core assumption ("a cross-tap `depends_on` gives a frictionless single
command") did not hold on Homebrew 6.

### Why it was reverted

Installing lgx with the `depends_on "nooga/tap/let-go"` formula fails on a
clean machine:

```
Error: Refusing to load formula nooga/tap/let-go from untrusted tap nooga/tap.
Run `brew trust --formula nooga/tap/let-go` or `brew trust nooga/tap` to trust it.
```

Homebrew 6 gates **transitive** dependencies pulled from an untrusted
third-party tap behind an explicit `brew trust` step. Trust is a user-side
decision and can't be pre-set from the formula. So the "single command"
became: hit a scary error → `brew trust nooga/tap` → re-run install. That
erodes the exact benefit `depends_on` was added for. Note the asymmetry:
a *direct* `brew install nooga/tap/let-go` (the user naming the tap) does
**not** trip the gate — only transitive deps do.

### Final approach (implemented)

Keep let-go a **user-installed direct dependency**, documented via the
formula's `caveats` and the repo docs, with the command modernized to the
single cross-platform `brew install nooga/tap/let-go` (nooga now ships
let-go as a proper formula on macOS and Linux). Same two commands on first
run, no error, no trust step, no coupling to nooga's tap for lgx's own
install to succeed.

- **Generator** (`scripts/generate-formula.sh`): no `depends_on`; `caveats`
  restored with the simplified `brew install nooga/tap/let-go` (dropped the
  old macOS/Linux split — one command now works on both). Passes
  `brew style` with 0 offenses.
- **Docs** (`README.md`, `scripts/README.md`, `scripts/install.sh`,
  `landing/index.html`): every let-go install hint updated from the old
  `brew tap nooga/let-go … && brew install let-go` to
  `brew install nooga/tap/let-go`. README Homebrew section states brew
  installs lgx only and points to the direct let-go install. No
  `brew tap nooga/let-go …` string remains anywhere.
- **Tap** (`homebrew-tap/README.md`, separate repo, uncommitted): the
  manual let-go line updated to the simplified command and correct
  `>= 1.11.0` (was a stale `1.10.0`).

### Net effect vs. the original goal

The `depends_on` goal is dropped; the durable win is that the *manual*
let-go install command is simplified and correct everywhere.

Note: `depends_on` already shipped in **v0.1.0-rc3** (merged via PR #32),
so the live tap formula currently carries it and errors on install. This
revert fixes the generator; the tap's `Formula/lgx.rb` only picks up the
fix when the **next release** (rc4+) regenerates it. Andrey cuts that
release and verifies the `brew install` flow.
